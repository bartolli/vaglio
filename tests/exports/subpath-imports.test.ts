 ...
 * are never exercised").
 *
 * Imports @bartolli/vaglio via its own package name (Node self-reference) so the
...
describe('@bartolli/vaglio root exports', () => {
  it('resolves the root subpath', async () => {
    const root = await import('@bartolli/vaglio');
...
describe('@bartolli/vaglio/unicode subpath', () => {
  it('exposes the composed pipeline + low-level helpers', async () => {
    const mod = await import('@bartolli/vaglio/unicode');
  ...
  it('does NOT expose internal granular strippers (deferred to v0.2)', async () => {
    const mod = (await import('@bartolli/vaglio/unicode')) as Record<string, unknown>;
  ...
  it('subpath stripUnicode and root stripUnicode are the same function', async () => {
    const sub = await import('@bartolli/vaglio/unicode');
    const root = await import('@bartolli/vaglio');
...
describe('@bartolli/vaglio/credentials subpath', () => {
  it('exposes redact + redactDetailed + DEFAULT_CREDENTIAL_PATTERNS', async () => {
    const mod = await import('@bartolli/vaglio/credentials');
  ...
  it('does NOT expose internal cross-module plumbing', async () => {
    const mod = (await import('@bartolli/vaglio/credentials')) as Record<string, unknown>;
  ...
  it('subpath redact and root redact are the same function', async () => {
    const sub = await import('@bartolli/vaglio/credentials');
    const root = await import('@bartolli/vaglio');
...
describe('@bartolli/vaglio/tags subpath', () => {
  it('exposes stripTags + stripTagsDetailed + DEFAULT_REASONING_TAGS', async () => {
    const mod = await import('@bartolli/vaglio/tags');
  ...
  it('subpath stripTags and root stripTags are the same function', async () => {
    const sub = await import('@bartolli/vaglio/tags');
    const root = await import('@bartolli/vaglio');
...
describe('subpath functional smoke (defense surface intact)', () => {
  it('@bartolli/vaglio/unicode stripUnicode strips zero-width via subpath', async () => {
    const { stripUnicode } = await import('@bartolli/vaglio/unicode');
    const zwsp = '​';
    const out = stripUnicode(`hel${zwsp}lo`);
    expect(out).toBe('hello');
  });

  it('@bartolli/vaglio/credentials redact via subpath', async () => {
    const { redact } = await import('@bartolli/vaglio/credentials');
    const out = redact(
  ...
  it('@bartolli/vaglio/tags stripTags via subpath', async () => {
    const { stripTags } = await import('@bartolli/vaglio/tags');
